...
def load_image_file(*, location):
    ...
    print(SimpleITK.GetArrayFromImage(result))
...
if __name__ == "__main__":
    ...
    # Get all pred mha files in the folder
    pred_files = glob("./output/images/pelvic-fracture-ct-segmentation/*.mha")
    print(pred_files)

    # Get all gt mha files in the folder
    gt_files = glob("./gt/*.mha")
    print(gt_files)

    # Init metrics_single_cases to store all metrics
    metrics_single_cases = []

    # Loop through each pair of pred and gt files
    for pred_file, gt_file in zip(pred_files, gt_files):
        # Print the pred and gt file names
        print(f"Pred file: {pred_file}")
        print(f"GT file: {gt_file}")

        # Load the ground truth label and spacing
        spacing, gt_volume = load_gt_label_and_spacing(Path(gt_file))

        # Load the predicted label
        pred_volume = load_image_file(location=pred_file)

        # Evaluate the segmentation results
        metrics_single_case = evaluate_3d_single_case(gt_volume, pred_volume, spacing)
        print(metrics_single_case)

        # Append the metrics to the list
        metrics_single_cases.append(metrics_single_case)

    # Save the metrics to a CSV file
    with open("metrics.csv", "a") as f:
        f.write("fracture_iou,fractrue_hd95,fracture_assd,anatomical_iou,anatomical_hd95,anatomical_assd\n")
        for metrics in metrics_single_cases:
            f.write(f"{metrics['fracture_iou']},{metrics['fracture_hd95']},{metrics['fracture_assd']},"
                    f"{metrics['anatomical_iou']},{metrics['anatomical_hd95']},{metrics['anatomical_assd']}\n")
            
    # Calculate average metrics for all cases
    avg_fracture_iou = sum(metrics['fracture_iou'] for metrics in metrics_single_cases) / len(metrics_single_cases)
    avg_fracture_hd95 = sum(metrics['fracture_hd95'] for metrics in metrics_single_cases) / len(metrics_single_cases)
    avg_fracture_assd = sum(metrics['fracture_assd'] for metrics in metrics_single_cases) / len(metrics_single_cases)
    avg_anatomical_iou = sum(metrics['anatomical_iou'] for metrics in metrics_single_cases) / len(metrics_single_cases)
    avg_anatomical_hd95 = sum(metrics['anatomical_hd95'] for metrics in metrics_single_cases) / len(metrics_single_cases)
    avg_anatomical_assd = sum(metrics['anatomical_assd'] for metrics in metrics_single_cases) / len(metrics_single_cases)

    # Print the average metrics
    print(f"Average fracture IoU: {avg_fracture_iou:.2f}")
    print(f"Average fracture HD95: {avg_fracture_hd95:.2f}")
    print(f"Average fracture ASSD: {avg_fracture_assd:.2f}")
    print(f"Average anatomical IoU: {avg_anatomical_iou:.2f}")
    print(f"Average anatomical HD95: {avg_anatomical_hd95:.2f}")
